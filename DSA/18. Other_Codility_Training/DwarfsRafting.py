# https://codility.com/demo/take-sample-test/dwarfs_rafting/
"""
...
The raft is square and has N rows of seats, numbered from 1 to N. Each row contains N seats, labeled with consecutive letters (A, B, C, etc.).
Each seat is identified by a string composed of its row number followed by its column number; for example, "9C" denotes the third seat in the 9th row.

The raft has already been loaded with barrels in some seat positions, and other seats are already occupied by dwarfs.
Our company of dwarfs may only take the remaining unoccupied seats. The ferryman wants to accommodate as many dwarfs as possible,
but the raft needs to be stable when making the crossing. That is, the following conditions must be satisfied:
...
The contents of the strings are separated by single spaces and may appear in any order. For example, in the diagram above, S = "1B 1C 4B 1D 2A" and T = "3B 2D".
...
that, given the size of the raft N and two strings S, T that describes the positions of barrels and occupied seats, respectively,
returns the maximum number of dwarfs that can fit on the raft. If it is not possible to balance the raft with dwarfs, your function should return -1.
...
# Note: N is an even integer within the range [2..26];
# Raft is square
def solution(N, S, T):
    ...
    for barrel in S.split():
        ...
        row = int(barrel[:-1]) - 1
        column = ord(barrel[-1]) - ord("A")  # if A -> 0; if B -> 1
        if row < boundary:
    ...
    # print(quadrant_left_front, quadrant_left_back, quadrant_right_front, quadrant_right_back) #2,3,2,4
    """
    lf is short for left front,... so on
    To keep balance, we need:
        1. weight_lf + weight_lb = weigth_rf + weight_rb
        2. weight_lf + weight_rf = weight_lb + weight_rb
    Solve the equation and we can get the answer:
        1. weight_lb = weight_rf
        2. weight_lf = weight_rb
    """

    allowance_lf = min(quadrant_left_front, quadrant_right_back)  # 2
    allowance_rb = min(quadrant_left_front, quadrant_right_back)  # 2
    allowance_lb = min(quadrant_left_back, quadrant_right_front)  # 2
    allowance_rf = min(quadrant_left_back, quadrant_right_front)  # 2
    ...
    for dwarf in T.split():
        ...
        row = int(dwarf[:-1]) - 1
        column = ord(dwarf[-1]) - ord("A")  # if A -> 0; if B -> 1
        if row < boundary:
            ...
            if column < boundary:
                ...
                if allowance_rb < 0:
                    return -1
    return allowance_lf + allowance_rf + allowance_lb + allowance_rb
...
"""
return -1 if no solution possible
should return 6 this case
"""
