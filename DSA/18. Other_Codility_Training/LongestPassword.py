...
The goal is to choose the longest word that is a valid password. You can assume that if there are K spaces in string S then there are exactly K + 1 words.
...
def check_valid_pw(word):
    ...
    if (num_of_others == 0) and (num_of_letters % 2 == 0) and (num_of_digits % 2 != 0):
        return True
    else:
        return False
...
def solution(S):
    splitS = list(S.split())
    # print(splitS)
    max_length = -1  # if no valid password then return -1

    for word in splitS:
        if check_valid_pw(word) == True:
            length = len(word)
